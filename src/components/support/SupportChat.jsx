...
import { MessageCircle, X as XIcon, Send } from "lucide-react";
...
export default function SupportChat() {
  ...
  return (
    <>
      <div onClick={() => setOpen(!open)} style={{ position: "fixed", bottom: "calc(env(safe-area-inset-bottom) + 72px)", right: 16, zIndex: 9999, width: 60, height: 60, borderRadius: "50%", background: "linear-gradient(135deg, #1B5E20, #2E7D32)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", boxShadow: "0 4px 20px rgba(27,94,32,0.4)", transition: "transform 0.2s" }} onMouseEnter={e => e.currentTarget.style.transform = "scale(1.1)"} onMouseLeave={e => e.currentTarget.style.transform = "scale(1)"}>
        {open ? <XIcon size={22} color="white" /> : <MessageCircle size={26} color="white" strokeWidth={2.2} />}
      ...
      {open && (
        <div style={{ position: "fixed", bottom: "calc(env(safe-area-inset-bottom) + 144px)", right: 16, zIndex: 9998, width: "min(360px, calc(100vw - 48px))", height: "min(500px, calc(100vh - 120px))", borderRadius: 20, background: "white", boxShadow: "0 8px 40px rgba(0,0,0,0.18)", display: "flex", flexDirection: "column", overflow: "hidden", fontFamily: "system-ui, sans-serif" }}>
          <div style={{ background: "linear-gradient(135deg, #1B5E20, #2E7D32)", padding: "1rem 1.25rem", display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ width: 40, height: 40, borderRadius: "50%", background: "rgba(255,255,255,0.2)", display: "flex", alignItems: "center", justifyContent: "center" }}><MessageCircle size={20} color="white" /></div>
            <div>
