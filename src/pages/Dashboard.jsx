...
case "messages": navigate("/Messages"); return null;
  ...
  return (
    <>
      <style>{styles}</style>
            {/* ===== VUE MOBILE iOS ===== */}
      <div className="md:hidden" style={{minHeight:"100vh",background:"#F2F2F7",fontFamily:"-apple-system,system-ui,sans-serif"}}>
        <div style={{padding:"16px 20px 12px",fontSize:32,fontWeight:700,letterSpacing:0.3,color:"#000"}}>Profil</div>
        <div style={{margin:"0 16px 12px",background:"#fff",borderRadius:16,padding:"14px 16px"}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:17,fontWeight:600,color:"#000",marginBottom:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{profile?.shop_name||profile?.full_name||user?.email}</div>
              <div style={{fontSize:13,color:"rgba(60,60,67,0.6)",marginBottom:3}}>{user?.email}</div>
            </div>
            <button onClick={()=>setActiveSection("info")} style={{background:"#F2F2F7",border:"none",borderRadius:8,padding:"6px 14px",fontSize:14,fontWeight:500,color:"#000",cursor:"pointer"}}>Modifier</button>
          </div>
        </div>
        <div style={{margin:"0 16px 16px",background:"#fff",borderRadius:16,padding:"14px 8px"}}>
          <div style={{display:"flex",justifyContent:"space-around"}}>
            <div style={{textAlign:"center"}}><div style={{fontSize:20,fontWeight:700,color:"#000"}}>{profile?.sales_count||0}</div><div style={{fontSize:12,color:"rgba(60,60,67,0.6)",marginTop:2}}>Ventes</div></div>
            <div style={{width:"0.5px",background:"rgba(60,60,67,0.15)"}}></div>
            <div style={{textAlign:"center"}}><div style={{fontSize:20,fontWeight:700,color:"#000"}}>{profile?.purchases_count||0}</div><div style={{fontSize:12,color:"rgba(60,60,67,0.6)",marginTop:2}}>Achats</div></div>
            <div style={{width:"0.5px",background:"rgba(60,60,67,0.15)"}}></div>
            <div style={{textAlign:"center"}}><div style={{fontSize:20,fontWeight:700,color:"#000"}}>{profile?.favorites_count||0}</div><div style={{fontSize:12,color:"rgba(60,60,67,0.6)",marginTop:2}}>Favoris</div></div>
          </div>
        </div>
        <div style={{padding:"0 0 32px"}}>
          {NAV.map(group => (
            <div key={group.group}>
              <div style={{padding:"0 32px 6px",fontSize:12,fontWeight:500,color:"rgba(60,60,67,0.6)",textTransform:"uppercase",letterSpacing:0.5,marginTop:8}}>{group.group}</div>
              <div style={{margin:"0 16px 12px",background:"#fff",borderRadius:16,overflow:"hidden"}}>
                {group.items.map((item, i) => (
                  <div key={item.id} onClick={()=>setActiveSection(item.id)}
                    style={{display:"flex",alignItems:"center",padding:"0 16px",minHeight:50,position:"relative",cursor:"pointer",background:activeSection===item.id?"#f0f4f0":"#fff"}}>
                    <span style={{fontSize:18,marginRight:14,width:24,textAlign:"center"}}>{item.icon}</span>
                    <span style={{flex:1,fontSize:16,color:activeSection===item.id?"#1B5E20":"#000",fontWeight:activeSection===item.id?600:400}}>{item.label}</span>
                    <svg width="8" height="13" viewBox="0 0 8 13"><path d="M1 1l6 5.5-6 5.5" stroke="rgba(60,60,67,0.3)" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round"/></svg>
                    {i < group.items.length-1 && <div style={{position:"absolute",bottom:0,left:54,right:0,height:"0.5px",background:"rgba(60,60,67,0.12)"}}></div>}
                  </div>
                ))}
              </div>
            </div>
        ...
        {activeSection && (
          <div style={{position:"fixed",inset:0,background:"#F2F2F7",zIndex:200,overflowY:"auto"}}>
            <div style={{background:"#F2F2F7",padding:"56px 16px 12px",display:"flex",alignItems:"center",gap:12,borderBottom:"0.5px solid rgba(60,60,67,0.15)"}}>
              <button onClick={()=>setActiveSection(null)} style={{background:"rgba(116,116,128,0.12)",border:"none",borderRadius:9999,width:36,height:36,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}>
              ...
              <span style={{fontSize:17,fontWeight:600,color:"#000"}}>{NAV.flatMap(g=>g.items).find(i=>i.id===activeSection)?.label}</span>
            </div>
